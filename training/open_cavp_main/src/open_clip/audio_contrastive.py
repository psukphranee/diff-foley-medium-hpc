...
# X3D:
from .x3d import init_X3D, init_I3D

# R2plus:
# import sys
# sys.path.append("/public/MARS/Users/lsm/lsm_project/lsm_project/open_clip-main/src/open_clip/")
from .resnet2plus1d import init_R2plus

from .cnn10 import Cnn10, Cnn14
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, truncate_sec=4):
        ...
        assert truncate_sec == 4 or truncate_sec == 8
        if truncate_sec == 4:
...
def spec_encoder_resnet50(truncate_sec=4):
    """ return a ResNet 50 object
    """
    return ResNet(BottleNeck, [3, 4, 6, 3], truncate_sec=truncate_sec)



## def VIT Spec Backbone:
...
class Spec_VIT(nn.Module):
    ...
    def __init__(
        ...
        self.spec_size = spec_size      # default: 256 -> 4s
        self.patch_size = patch_size    # default: 16
        self.grid_size = spec_size // patch_size

        self.cls_token = cls_token  

        self.output_dim = output_dim
        self.conv1 = nn.Conv1d(in_channels=128, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)

        scale = width ** -0.5
        if self.cls_token:
            self.class_embedding = nn.Parameter(scale * torch.randn(width))
            self.positional_embedding = nn.Parameter(scale * torch.randn(self.grid_size + 1, width))
        else:
            self.positional_embedding = nn.Parameter(scale * torch.randn(self.grid_size, width))
    ...
    def forward(self, x: torch.Tensor):
        x = self.conv1(x)  #  B x C x patch_num x patch_num
        x = x.reshape(x.shape[0], x.shape[1], -1)  #  B x C x L
...
if __name__ == "__main__":
    ...
    # base = {"spec_size":256, "patch_size":16, "width":768, "layers":12, "heads":12, "mlp_ratio":4, "output_dim": 768}
    # medium = {"spec_size":256, "patch_size":16, "width":1024, "layers":16, "heads":16, "mlp_ratio":4, "output_dim": 1024}
    # large = {"spec_size":256, "patch_size":16, "width":1024, "layers":24, "heads":16, "mlp_ratio":4, "output_dim": 1024}

    # model = Spec_VIT(**large)

    # count = 0
    # for p in model.parameters():
    #     count += p.numel()
    # print("Parmas Num: ", count)

    # x = torch.randn(2,128,256)
    # pooled, tokens = model(x)
    # # print(out.shape)
    # print(tokens.shape)
    # print(pooled.shape)


    # model = spec_encoder_resnet50(truncate_sec=4)
    # x = torch.randn(2, 1, 128, 256)
    # out = model(x)
    # print(out.shape)


    # model = Spec_VIT(spec_size=256, patch_size=16, width=512, layers=12, heads=8, mlp_ratio=4)
    # count = 0
    # for p in model.parameters():
    #     count += p.numel()
    # print("Parmas Num: ", count)