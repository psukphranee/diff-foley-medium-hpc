...
def group_by_keys_nothrow(data, keys=base_plus_ext, lcase=True, suffixes=None, handler=None):
    ...
    for filesample in data:
        ...
        print(f"Panya: count={count}, fname={fname}, current_sample keys={current_sample.keys() if current_sample else ''}")
...
def get_wds_dataset_vggsound_audioset_music_intra_contrast(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    if is_train and args.train_data == "audioset_vggsound_music":
        ...
        input_shards = [os.path.join(diff_foley_main, "video/goodarchive_{}.tar").format(str(i)) for i in range(1,2)]
