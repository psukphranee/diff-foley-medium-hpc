...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # spec:  sample_num x H x W
        # video: sample_num x T x C x H x W
        spec, video, video_time = batch          
        ...
        if args.accum_freq == 1:
            ...
            with torch.no_grad():
                with autocast():
                    model_out = model(images, texts, train=True)
    ...
    if 'val' in data and (args.val_frequency and ((epoch % args.val_frequency) == 0 or epoch == args.epochs)):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                # images = batch['video']
                # texts = batch['spec']
                texts, images, video_time = batch
                images = images.to(device=device, dtype=cast_dtype, non_blocking=True)
                texts = texts.to(device=device, non_blocking=True)

                with autocast():
                    model_out = model(images, texts, output_dict=True, train=False)
                    ...
                    # bs = logits_per_image.shape
                    # s = (range(bs // clip_num), np.s_[:], range(bs // clip_num), np.s_[:])
                    # intra_logits_per_image = logits_per_image.reshape(bs//clip_num, clip_num, bs//clip_num, clip_num)[s]   # b' x clip_num x clip_num
                    # intra_logits_per_text = logits_per_text.reshape(bs//clip_num, clip_num, bs//clip_num, clip_num)[s]     # b' x clip_num x clip_num
                    # bs_intra, num_logits_intra, _ = intra_logits_per_image.shape
                    # # Intra Logits Per Image: bs x c
                    # intra_logits_per_image = intra_logits_per_image.reshape(bs*num_logits_intra, num_logits_intra)
                    # intra_logits_per_text = intra_logits_per_text.reshape(bs*num_logits_intra, num_logits_intra)
                    ...
                    total_loss = extra_contrast_loss

                    gen_loss = maybe_compute_generative_loss(model_out)

                cumulative_loss += total_loss * batch_size
                # cumulative_extra_loss += extra_contrast_loss * batch_size
                # cumulative_intra_loss += intra_contrast_loss * batch_size
            ...
            # loss_intra = cumulative_intra_loss / num_samples
            # loss_extra = cumulative_extra_loss / num_samples
